...
if(request.getParameter("msg") != null){
	...
	<div class="alert alert-danger alert-dismissible fade show text-center h5 " style="font-size: 1.3vw" role="alert"><strong>Oops...!</strong> Please select different products to build your PC Otherwise you are not able to build your PC<button type="button" class="close p-0" data-dismiss="alert"  aria-label="Close"><span aria-hidden="true">&times;</span></button></div>
<% } else if(request.getParameter("msg").equals(" regSuc")){%>
	<div class="alert alert-success alert-dismissible fade show text-center h5" role="alert"><strong>You are Successful to build your Own PC ...!</strong><button type="button" class="close" data-dismiss="alert p-0" aria-label="Close"><span aria-hidden="true">&times;</span></button></div><!-- <i class="fa fa-info-circle"></i> -->
<% }
 }
%>
	...
	<div class="alert alert-info alert-dismissible fade show text-center h5" role="alert"><strong class="font-weight-bold" style="font-size: 1.2vw ! important">
	For build your own PC you have must select 1 item from every category else your order is disqualified and cancle.</strong>
	<button type="button" class="close p-0" data-dismiss="alert"  aria-label="Close"><span aria-hidden="true">&times;</span></button></div>
				...
				<div class="sidebar p-3">					
					...
					<div class="side_cat mt-3">
						...
						<div class="form-group h6 mt-3 text-center">
							<button class="btn btn-danger" type="button" onclick='if(confirm("Do you want to Refresh ?")){ location.href="CustomizeProductById.jsp" }'>Reset Filter</button>
						</div>
						<% int cnt = CustListDao.getCount(ub.getUserid());
							if( (cnt==5) || (cnt ==6) )
							{
						%>
							<div class="form-group h6 mt-3 text-center">
								<form method="post" action="<%=application.getContextPath()%>/CustListController" >
								<input class="btn btn-danger" type="submit" name="action" value="Go To List">	
								</form>
						...
						%>
							<div class="form-group h6 mt-3 text-center">
								<button class="btn btn-danger" type="button" onclick="GoToList();" disabled>Go To List</button>
							</div>
						<%
							}
						%>
						...
						<div class="side animated wow slideInUp">
							<div class="img">
								<form method="post" action="<%=application.getContextPath()%>/CustProdController" >
										...
										<button type="hidden" type="submit" name="action" value="customquickviewById" class="btn btn-white">									
							...
							<%	 
									...
									boolean t = CustListDao.getListByProUserId(cust.getCustproid(),ub.getUserid());
									if(!t)
									{
									%> 																			
									<form action="<%=application.getContextPath()%>/CustListController"  method="post">
		    	       	  				...
		    	       	  				<input type="hidden" name="catid" value="<%=cust.getCustmaincatid()%>">                	  	
									...
									<form action="<%=application.getContextPath()%>/CustListController"  method="post">
		    	       	  				...
		    	       	  				<input type="hidden" name="qty" value="1" min="1" max="50">
		    	       	  				<input type="hidden" name="catid" value="<%=cust.getCustmaincatid()%>">                	  	
										<input type="submit" class="item_add btn btn-danger float-left" name="action" value="Added to List">
									</form>	
									<%
									}							
								%>																			
